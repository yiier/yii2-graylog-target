...
class MessageBuilder extends BaseObject implements MessageBuilderInterface
{
    ...
    public function build(Target $target, $message)
    {
        ...
        if (($target->addUsername) && (Yii::$app->has('user')) && ($user = Yii::$app->get('user')) && ($identity = $user->getIdentity(false))) {
            $gelfMessage->setAdditional('username', $identity->username);
        }

        if ($target->additionalFields && is_array($target->additionalFields)) {
            $gelfMessage = $this->fillMessageWithAdditionalFields($gelfMessage, $target->additionalFields);
        }
     ...
     * @return Message
     */
    private function fillMessageWithAdditionalFields(Message $message, array $additionalFields)
    {
        foreach ($additionalFields as $key => $value) {
            if (is_string($key) && !empty($key)) {
                if (is_callable($value)) {
                    $value = call_user_func($value, Yii::$app);
                }
